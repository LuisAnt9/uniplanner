...
exports.createSubject = async (req, res) => {
  try {
    ...
    await logAction(req, { action: "criar_materia", category: "materia", details: { name: subject.name } });
...
exports.updateSubject = async (req, res) => {
  try {
    ...
    await logAction(req, { action: "editar_materia", category: "materia", details: { name: subject.name } });
...
exports.deleteSubject = async (req, res) => {
  try {
    ...
    if (!subject) return res.status(404).json({ success: false, error: "Matéria não encontrada" });
    await logAction(req, { action: "deletar_materia", category: "materia", details: { name: subject.name } });
